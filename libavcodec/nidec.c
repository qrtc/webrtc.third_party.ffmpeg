...
int xcoder_decode_close(AVCodecContext *avctx)
{
  ...
  s->extradata_size = 0;
  s->got_first_idr = 0;
  s->dev_xcoder = NULL; //Set to Null, else av_opt_free in avcodec_close will try to free it and crash as "string literal" cannot be freed.
...
int xcoder_decode_init(AVCodecContext *avctx)
{
  ...
  av_log(avctx, AV_LOG_VERBOSE, "width: %d height: %d pix_fmt: %s\n",
  ...
  if (s->dev_dec_idx < 0)
  {
    // the user did not specify which device to use.
    // we need to auto select the device,
    // we need to know which selection method to use.
    // if the slection method is not in place, we use best model by default
    //
    if (strlen(s->dev_xcoder)<=0)
    {
      strcpy(s->api_ctx.dev_xcoder, BEST_MODEL_LOAD_STR);
    }
    else
    {
